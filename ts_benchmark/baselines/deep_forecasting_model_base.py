...
DEFAULT_HYPER_PARAMS = {
    ...
    "alpha_cov": 1.0,
    "mlp_hidden_dims": 64,
    "profile_train_efficiency": False,
    "profile_warmup_iters": 5,
    "selective_input": False,
}
...
class DeepForecastingModelBase(ModelBase):
    ...
    def _init_optimizer(self, CovariateFusion=None):
        ...
        if hasattr(self, "CovariateFusion") and self.CovariateFusion is not None:
            return optim.Adam(
                [
                    {"params": self._trainable_parameters(self.model), "lr": self.config.lr},
                    {"params": self._trainable_parameters(self.CovariateFusion), "lr": self.config.lr},
                ]
            )
        else:
            return optim.Adam(self._trainable_parameters(self.model), lr=self.config.lr)
    ...
    def _selective_input_enabled(self) -> bool:
        return bool(getattr(self.config, "selective_input", False))

    def _mask_history_exog_tensor(
        self, data: torch.Tensor, series_dim: int
    ) -> torch.Tensor:
        if (
            not self._selective_input_enabled()
            or data is None
            or data.shape[-1] <= series_dim
        ):
            return data
        data = data.clone()
        data[..., series_dim:] = 0.0
    ...
    def _mask_decoder_history_exog_tensor(
        self, data: torch.Tensor, series_dim: int, horizon: int
    ) -> torch.Tensor:
        if (
            not self._selective_input_enabled()
            or data is None
            ...
            return data
        data = data.clone()
        history_len = max(data.shape[1] - int(horizon), 0)
        if history_len > 0:
            data[:, :history_len, series_dim:] = 0.0
        return data

    def _mask_history_exog_numpy(
        self, data: np.ndarray, series_dim: int
    ) -> np.ndarray:
        if (
            not self._selective_input_enabled()
            or data is None
    ...
    def _get_eval_missing_spec(self) -> Tuple[str, str]:
        variate = str(
            getattr(self.config, "eval_missing_variate", "none") or "none"
        ).lower()
        strategy = str(
            getattr(self.config, "eval_missing_strategy", "none") or "none"
        ).lower()

        variate_aliases = {
            "endo": "history_endogenous",
            "endogenous": "history_endogenous",
            "history_endo": "history_endogenous",
            "history_endogenous": "history_endogenous",
            "hist_endo": "history_endogenous",
            "hist_endogenous": "history_endogenous",
            "exo": "future_exogenous",
            "exogenous": "future_exogenous",
            "future_exo": "future_exogenous",
            "future_exogenous": "future_exogenous",
            "history_exo": "history_exogenous",
            "history_exogenous": "history_exogenous",
            "hist_exo": "history_exogenous",
            "hist_exogenous": "history_exogenous",
            "none": "none",
            "": "none",
            "false": "none",
        }
        ...
        variate = variate_aliases.get(variate, variate)
        strategy = strategy_aliases.get(strategy, strategy)
        if variate == "none" or strategy == "none":
            return "none", "none"
        if variate not in {
            "history_endogenous",
            "future_exogenous",
            ...
            raise ValueError(f"Unsupported eval_missing_variate: {variate}")
        if strategy not in {"zeros", "random"}:
            raise ValueError(f"Unsupported eval_missing_strategy: {strategy}")
        return variate, strategy

    def _eval_missing_rng(self, batch_index: int, salt: int) -> np.random.Generator:
        seed = int(getattr(self.config, "eval_missing_seed", 2021))
        return np.random.default_rng(seed + int(batch_index) * 104729 + salt)

    def _apply_eval_missing_to_numpy(
        self,
        data: np.ndarray,
        variate: str,
        start_channel: int,
        ...
        missing_variate, missing_strategy = self._get_eval_missing_spec()
        if (
            missing_variate != variate
            or data is None
            or start_channel >= end_channel
        ):
            return data

        data = data.copy()
        target = data[..., start_channel:end_channel]
        if missing_strategy == "zeros":
            target[...] = 0.0
        else:
            rng = self._eval_missing_rng(batch_index, salt)
            target[...] = rng.random(target.shape).astype(target.dtype, copy=False)
        return data

    def _apply_eval_missing_to_tensor(
        self,
        data: torch.Tensor,
        variate: str,
        batch_index: int,
        ...
        missing_variate, missing_strategy = self._get_eval_missing_spec()
        if missing_variate != variate or data is None:
            return data
        ...
        rng = self._eval_missing_rng(batch_index, salt)
        values = rng.random(tuple(data.shape)).astype(np.float32, copy=False)
        return torch.as_tensor(values, device=data.device, dtype=data.dtype)

    def _get_eval_future_exog_shift(self) -> int:
        shift = getattr(self.config, "eval_future_exog_shift", 0) or 0
        return int(shift)

    @staticmethod
    def _shift_future_exog_window(data: np.ndarray, shift: int) -> np.ndarray:
        if data is None or shift == 0:
            return data
        length = data.shape[1]
        if length == 0:
            return data

        offset = min(abs(int(shift)), length)
        shifted = np.empty_like(data)
        if shift > 0:
            shifted[:, : length - offset, :] = data[:, offset:, :]
            shifted[:, length - offset :, :] = data[:, -1:, :]
        else:
            shifted[:, offset:, :] = data[:, : length - offset, :]
            shifted[:, :offset, :] = data[:, :1, :]
        return shifted

    def _get_eval_future_exog_window(
        self,
        exog_futures,
        ...
        if exog_futures is None:
            return None

        start = batch_index * batch_size
        default_window = np.asarray(exog_futures[start : start + batch_size, -horizon:, :])
        shift = self._get_eval_future_exog_shift()
        if shift == 0:
            return default_window

        eval_batch_maker = getattr(batch_maker, "_batch_maker", None)
        covariates = getattr(eval_batch_maker, "covariates", {}) or {}
        index_list = getattr(eval_batch_maker, "index_list", None)
        exog_data = covariates.get("exog") if isinstance(covariates, dict) else None
        if exog_data is None or index_list is None:
            return self._shift_future_exog_window(default_window, shift)

        exog_values = exog_data.values if hasattr(exog_data, "values") else np.asarray(exog_data)
        sample_count = default_window.shape[0]
        shifted_starts = np.asarray(index_list[start : start + sample_count], dtype=int) + shift
        max_start = exog_values.shape[0] - horizon
        if max_start < 0:
            return self._shift_future_exog_window(default_window, shift)

        shifted_starts = np.clip(shifted_starts, 0, max_start)
        return np.stack(
            [exog_values[start_idx : start_idx + horizon] for start_idx in shifted_starts],
            axis=0,
        )
    ...
    def validate(
        ...
        with torch.no_grad():
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                model_input = self._mask_history_exog_tensor(input, series_dim)
                model_target = self._mask_decoder_history_exog_tensor(
                    target, series_dim, config.horizon
                )
                out_loss = self._process(
                    model_input, model_target, input_mark, target_mark, exog_future
                )
                additional_loss = 0
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        profile_train_efficiency = bool(
            getattr(config, "profile_train_efficiency", False)
        )
        profile_warmup_iters = int(getattr(config, "profile_warmup_iters", 5))
        profile_step_times = []
        profile_total_iters = 0
        if profile_train_efficiency and device.type == "cuda":
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                if profile_train_efficiency and profile_step_start is not None:
                    if device.type == "cuda":
                        torch.cuda.synchronize(device)
                    elapsed_ms = (time.perf_counter() - profile_step_start) * 1000.0
                    if profile_total_iters >= profile_warmup_iters:
                        profile_step_times.append(elapsed_ms)
                    profile_total_iters += 1
        ...
        if profile_train_efficiency:
            ...
            self.efficiency_profile_ = {
                "train_ms_per_iter": (
                    float(np.mean(profile_step_times)) if profile_step_times else None
                ),
                "peak_memory_mb": peak_memory_mb,
                "profile_steps": len(profile_step_times),
                "profile_warmup_iters": profile_warmup_iters,
                "total_iterations": profile_total_iters,
                "device": str(device),
            }
        else:
            self.efficiency_profile_ = None
    ...
    def batch_forecast(
        ...
        input_np = self._apply_eval_missing_to_numpy(
            input_np,
            "history_endogenous",
            0,
            series_dim,
        ...
        input_np = self._mask_history_exog_numpy(input_np, series_dim)

        # 传入exog_futures,每个batch中对应的未来协变量
        exog_future_np = self._get_eval_future_exog_window(
            exog_futures,
            batch_maker,
            i,
            batch_size,
        ...
        if exog_future_np is not None:
            exog_future = torch.tensor(exog_future_np).to(device)
        else:
            exog_future = None
        ...
        exog_future = self._apply_eval_missing_to_tensor(
            exog_future,
            "future_exogenous",
            i,
            salt=23,
    ...
    def _perform_rolling_predictions(
        ...
        with torch.no_grad():
            ...
            while not answers or sum(a.shape[1] for a in answers) < horizon:
                ...
                input = self._mask_history_exog_tensor(input, series_dim)
                dec_input = self._mask_decoder_history_exog_tensor(
                    dec_input, series_dim, self.config.horizon
                )
                if exog_future is not None:
